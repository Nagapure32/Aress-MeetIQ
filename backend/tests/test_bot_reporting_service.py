...
def test_record_bot_transcript_can_store_encrypted_text(monkeypatch):
    from app.services import bot_reporting
    ...
    monkeypatch.setattr(
        bot_reporting,
        "protect_transcript_text",
        lambda text: {
            "text": "[encrypted transcript]",
            "encrypted_text": f"cipher:{text}",
            "encryption_alg": "test-v1",
    ...
    _path, rows = fake.insert_calls[0]
    assert isinstance(rows, list)
    assert rows[0]["text"] == "[encrypted transcript]"
    assert rows[0]["encrypted_text"] == "cipher:confidential launch details"
    assert rows[0]["encryption_key_id"] == "test-key"
