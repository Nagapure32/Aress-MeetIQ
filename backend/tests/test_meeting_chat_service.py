...
def test_get_search_client_selects_chroma_provider(monkeypatch):
    from app.services import meeting_chat

    monkeypatch.setattr(meeting_chat.settings, "vector_store_provider", "chroma", raising=False)

    search_client = meeting_chat._get_search_client()

    assert isinstance(search_client, meeting_chat.ChromaMeetingSearchClient)


def test_chroma_client_uploads_and_searches_meeting_scoped_chunks(monkeypatch):
    from app.services import meeting_chat
    ...
    class FakeCollection:
        ...
        def query(self, **kwargs) -> dict:
            ...
            return {
                "ids": [["meeting-1-0000"]],
                "documents": [["Asha: The launch stays on Friday."]],
                "metadatas": [
                    [
                        {
                            "meeting_id": "meeting-1",
    ...
    client = meeting_chat.ChromaMeetingSearchClient()
    monkeypatch.setattr(client, "_get_collection", lambda: fake_collection)
    monkeypatch.setattr(client, "_embed_texts", fake_embed_texts)
    ...
    assert fake_collection.upsert_payload == {
        "ids": ["meeting-1-0000"],
        "documents": ["Asha: The launch stays on Friday."],
    ...
    assert fake_collection.query_payload == {
        "query_embeddings": [[1.0, 1.0, 1.0]],
        "n_results": 3,
        "where": {"$and": [{"meeting_id": "meeting-1"}, {"user_id": "user-1"}]},
