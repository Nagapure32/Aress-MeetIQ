...
async def index_meeting_transcript(meeting_id: str) -> dict[str, Any]:
    _ensure_ai_chat_enabled()
...
async def get_meeting_chat_messages(meeting_id: str) -> list[dict[str, Any]]:
    user_id = get_dev_user_id()
...
async def get_meeting_chat_index_status(meeting_id: str) -> dict[str, Any]:
    user_id = get_dev_user_id()
    await _get_owned_meeting(meeting_id, user_id)
    rows = await supabase_gateway.get(
        "meeting_ai_indexes",
...
async def chat_with_meeting_transcript(meeting_id: str, message: str) -> dict[str, Any]:
    _ensure_ai_chat_enabled()
