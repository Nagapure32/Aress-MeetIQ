...
async def list_user_meetings(transcript_ready: bool = False) -> list[dict[str, Any]]:
    ...
    meetings = await _safe_get(
        "meetings",
        {
            "select": (
                ...
                "status,bot_status,approval_status,created_at,updated_at"
            ),
...
async def get_user_meeting(meeting_id: str) -> dict[str, Any] | None:
    user_id = get_dev_user_id()
...
async def list_meeting_transcript(meeting_id: str) -> list[dict[str, Any]]:
...
async def get_meeting_summary(meeting_id: str) -> dict[str, Any] | None:
    rows = await _safe_get(
        "meeting_summaries",
        {
...
async def list_meeting_tasks(meeting_id: str) -> list[dict[str, Any]]:
    return await _safe_get(
        "tasks",
        {
            "select": (
