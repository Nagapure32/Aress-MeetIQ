...
async def list_user_tasks() -> list[dict[str, Any]]:
    ...
    return await _hydrate_tasks(list(tasks_by_id.values()))


async def create_user_task(payload: TaskCreate) -> dict[str, Any]:
    user_id = get_dev_user_id()
    ...
    return (await _hydrate_tasks([task]))[0]


async def update_user_task(task_id: str, payload: TaskUpdate) -> dict[str, Any]:
    user_id = get_dev_user_id()
...
async def delete_user_task(task_id: str) -> None:
    user_id = get_dev_user_id()
...
async def _hydrate_tasks(tasks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    tasks_with_assignees = await _with_assignees(tasks)
    return await _with_meetings(tasks_with_assignees)
...
async def _with_meetings(tasks: list[dict[str, Any]]) -> list[dict[str, Any]]:
