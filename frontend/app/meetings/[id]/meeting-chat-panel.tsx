...
export function MeetingChatPanel({ meetingId, transcriptCount }: Props) {
  ...
  const statusLabel = formatStatusLabel(indexStatus?.status ?? "loading");
  ...
  return (
    <div className="flex h-full min-h-[520px] flex-col">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div
          className="inline-flex size-9 items-center justify-center rounded-md border border-line bg-white"
          title={`Chat index status: ${statusLabel}`}
        >
          <span
            className={`size-2.5 rounded-full ${chatIndexStatusDotClass(indexStatus?.status, empty)}`}
            aria-hidden="true"
          />
          <span className="sr-only">Chat index status: {statusLabel}</span>
        </div>
        <button
          type="button"
          onClick={handleIndex}
          disabled={indexing || transcriptCount === 0}
          className="grid size-9 place-items-center rounded-md border border-line bg-white text-ink transition hover:border-brand hover:bg-brand-soft disabled:cursor-not-allowed disabled:opacity-55"
          aria-label={ready ? "Reindex chat" : "Prepare chat"}
          title={ready ? "Reindex chat" : "Prepare chat"}
        >
      ...
      {error ? (
        <div className="mb-4 rounded-md border border-[#f0dfb5] bg-[#fff5d8] p-3 text-xs leading-5 text-[#8a5d00]">
          {error}
        </div>
      ) : null}
      ...
      <div className="mt-5 border-t border-line pt-4">
        <div className="flex gap-3">
          <input
            ...
            className="h-10 flex-1 rounded-md border border-line bg-shell px-4 text-xs text-ink outline-none transition focus:border-brand focus:bg-white disabled:cursor-not-allowed disabled:opacity-60"
          ...
          <button
            ...
            onClick={handleSend}
            disabled={!canSend}
            className="grid h-10 w-12 place-items-center rounded-md bg-brand text-white transition hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-55"
...
function ChatBubble({
  ...
  return (
    <div className={`flex ${assistant ? "justify-start" : "justify-end"}`}>
      <div
        className={`max-w-[720px] rounded-lg border p-4 text-sm leading-6 ${
          assistant ? "border-line bg-white text-ink" : "border-brand bg-brand text-white"
        }`}
      >
        <p>{content}</p>
...
function chatIndexStatusDotClass(status?: string | null, empty = false) {
  const normalized = status?.toLowerCase() ?? "loading";
  if (normalized === "ready") {
    return "bg-[#16a34a]";
  }
  if (normalized === "failed" || normalized === "empty" || empty) {
    return "bg-[#d97706]";
  }
  if (normalized === "indexing" || normalized === "loading") {
    return "bg-brand";
  }
  return "bg-muted";
}
