      ...
      <div className="mx-auto w-full max-w-[1480px] px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 border-b border-line pb-5 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap gap-3">
            ...
            <Link
              href="/meetings"
              className="flex h-9 items-center gap-2 rounded-md border border-brand bg-brand px-4 text-xs font-medium text-white transition hover:border-brand-dark hover:bg-brand-dark"
            >
              <Video size={13} />
              Meetings
            </Link>
        ...
        <section className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
          <div className="min-w-0 space-y-5">
            <PanelShell
              ...
              action={
                <Link href="/meetings" className="text-[11px] font-semibold text-brand-dark hover:text-brand">
                  View all
                </Link>
              }
              bodyClassName="max-h-[360px] overflow-y-auto pr-1"
            >
                ...
                <div className="divide-y divide-line">
                  {dashboard.upcoming_meetings.map((meeting) => {
                    ...
                    return (
                      <div
                        key={meeting.id}
                        className="grid gap-3 py-3 md:grid-cols-[104px_minmax(0,1fr)_auto]"
                      >
                        <div className="rounded-md bg-shell px-3 py-2 font-mono text-[11px] leading-5 text-muted">
                          <p className="font-semibold text-olive">{formatTime(meeting.start_time)}</p>
                ...
                <div className="space-y-3">
                  {recentActivity.map((activity) => (
                    <div
                      ...
                      className="flex items-start gap-3 rounded-lg border border-line bg-shell p-3 transition hover:border-brand hover:bg-brand-soft"
                    >
          ...
          <aside className="min-w-0 space-y-5">
            <PanelShell
              ...
              className="flex flex-col"
              bodyClassName="max-h-[520px] overflow-y-auto pr-1"
            >
              {!hasLiveData ? (
                <EmptyState
                ...
                <div className="space-y-3">
                  ...
                  {attentionItems.map((item) => (
                    <Link
                      ...
                      className="block rounded-lg border border-line p-3 transition hover:border-brand hover:bg-brand-soft"
                    >
                      <div className="flex items-start gap-2">
                        <AlertTriangle size={15} className="mt-0.5 shrink-0 text-brand-dark" />
                        <div className="min-w-0">
  ...
  return (
    <div className="rounded-lg border border-line bg-white p-4 shadow-panel">
      <div className="flex items-start justify-between gap-3">
        <p className="text-[11px] font-medium uppercase text-muted">{metric.label}</p>
        <span className="grid size-8 shrink-0 place-items-center rounded-md bg-brand-soft text-brand-dark">
          <Icon size={15} />
        </span>
      </div>
      <p className="mt-3 text-[28px] font-semibold leading-none text-ink">{metric.value}</p>
      <p className={`mt-2 min-h-4 font-mono text-[11px] leading-4 ${metricHelperClass(metric)}`}>
        {metric.helper ?? "Ready"}
      </p>
    </div>
  );
...
function AttentionTaskCard({ task }: { task: TaskItem }) {
  ...
  return (
    <article
      tabIndex={0}
      className="group rounded-lg border border-line bg-white p-3 outline-none transition hover:border-brand hover:bg-brand-soft focus:border-brand focus:bg-brand-soft"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
      ...
      <div className="mt-3 hidden rounded-lg border border-line bg-white p-3 text-xs group-hover:block group-focus:block">
        ...
        <Link
          href="/tasks"
          className="mt-3 inline-flex h-8 items-center justify-center gap-1 rounded-md bg-brand px-3 text-[11px] font-medium text-white transition hover:bg-brand-dark"
        >
...
function metricIcon(label: string) {
  ...
  if (value.includes("follow") || value.includes("task")) {
    return ListChecks;
  }
  if (value.includes("rate") || value.includes("completion")) {
    return TrendingUp;
  }
  if (value.includes("overdue") || value.includes("attention")) {
