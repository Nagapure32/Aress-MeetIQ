...
import { Panel, StatusDot, StatusPill } from "@/components/ui";
import {
  deleteTask,
  type TaskAssignee,
  type TaskItem,
...
export function TaskBoard({ initialTasks }: { initialTasks: TaskItem[] }) {
  ...
  const filteredStatusCounts = useMemo(() => buildStatusCounts(filteredTasks), [filteredTasks]);
    ...
    <div className="mt-8 space-y-4">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        >
          ...
          />
          <StatusOverviewBar counts={filteredStatusCounts} total={filteredTasks.length} />

          <div className="mt-4 rounded-lg border border-line">
            <div className="overflow-x-auto">
              <div className="grid min-w-[660px] grid-cols-[minmax(240px,1fr)_92px_84px_120px_36px] gap-3 border-b border-line bg-shell px-3 py-2 font-mono text-[10px] uppercase tracking-[0.06em] text-muted">
                <span>Task</span>
                <span>Priority</span>
                <span>Due</span>
                <span>Owner</span>
          ...
          <TaskPagination
            ...
            pageStart={filteredTasks.length ? pageStart + 1 : 0}
            pageEnd={Math.min(pageStart + TASKS_PER_PAGE, filteredTasks.length)}
...
function TaskPagination({
  ...
  return (
    <div className="mt-4 flex flex-col gap-3 rounded-lg border border-line bg-shell px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-xs text-muted">
        Showing <span className="font-medium text-ink">{pageStart}-{pageEnd}</span> of{" "}
        <span className="font-medium text-ink">{totalItems}</span> tasks
      </p>
...
function StatusOverviewBar({
  ...
  return (
    <div className="mt-4 rounded-lg border border-line bg-shell px-3 py-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        <p className="font-mono text-[10px] uppercase tracking-[0.06em] text-muted">Status mix</p>
        <p className="text-[11px] text-muted">{total} tasks</p>
      </div>
      <div className="flex h-2 overflow-hidden rounded-full bg-brand-soft" aria-label="Task status distribution">
        {statuses.map((status) => {
          const value = counts[status.value];
          if (!value) {
            return null;
          }
          return (
            <span
              ...
              className={`h-full ${statusBarClass(status.value)}`}
              style={{ width: `${Math.max(4, (value / Math.max(total, 1)) * 100)}%` }}
            />
          );
        })}
      </div>
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2">
        {statuses.map((status) => (
          <span key={status.value} className="inline-flex items-center gap-2 text-[11px] text-muted">
            <span className={`h-1.5 w-5 rounded-full ${statusBarClass(status.value)}`} aria-hidden="true" />
            {status.label}
          </span>
        ))}
      </div>
...
function TaskRow({
  ...
  return (
    <div
      ...
      className={`relative grid w-full min-w-[660px] grid-cols-[minmax(240px,1fr)_92px_84px_120px_36px] items-center gap-3 border-b border-line px-3 py-3 pl-4 text-left last:border-b-0 ${
        selected ? "bg-brand-soft" : "bg-panel hover:bg-shell"
      }`}
      title={`Status: ${statusLabel(task.status)}`}
    >
      <span className={`absolute left-0 top-0 h-full w-1 ${statusBarClass(task.status)}`} aria-hidden="true" />
      ...
      <span className="inline-flex items-center gap-2 text-xs text-muted" title={priorityLabel(task.priority)}>
        <StatusDot tone={priorityTone(task.priority)} label={priorityLabel(task.priority)} />
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 block text-[11px] text-muted">
                {formatMeetingDate(task.meeting.start_time)}
              </span>
            </span>
...
function buildStatusCounts(tasks: TaskItem[]): Record<TaskStatus, number> {
  return tasks.reduce(
    (counts, task) => {
